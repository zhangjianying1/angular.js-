...
.directive('verificationCode', ['AjaxService', '$timeout', '$parse', function(AjaxService, $timeout, $parse){
    return {
        restrice: 'AE',
        transclude: true,
        templateUrl: './includes/verificationcode.html',
        link: function(scope, ele, attrs){
            console.log(scope.defaults)

            scope.getCode = function(val){

                if (scope.defaults.disabled && !val) {
                    scope.defaults.btnText = '获取中...';
                    scope.defaults.disabled = false;
                    AjaxService.ajax('get', '/package.json').then(function(data){
                        if (data.name === 'agent') {
                            scope.defaults.btnText = '60'
                            setIntervalTime();
                        }
                    }, function(err){
                        if (err) {
            ...
            function setIntervalTime(){
                $timeout(function(){
                    scope.defaults.btnText--;
                    if (scope.defaults.btnText > 0) {
                        setIntervalTime()
                    } else {
                        scope.defaults.btnText = '获取验证码';
                        scope.defaults.disabled = true;
                        $timeout.cancel();
                    }
                }, 1000)
            }
 ...
 * 下拉刷新加载最新数据
 */
.directive('dorpdownload', ['$timeout', '$window', function($timeout, $window){
    return {
        restrict: 'AE',
        transclude: true,
        scope: {
        ...
        template: '<div><div style="width:100%;position:absolute; top:-50px; height:50px; left:0; background:#f90">{{ dorpText }}</div><div ng-transclude class="dorpcont" ></div><div class="footer-load" style="height:0; overflow:hidden">load</div></div>',
        link: function(scope, ele, attrs){
            var options = {
                element: ele[0],
